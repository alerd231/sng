...
- После загрузки сервер сохраняет файл в `public/uploads` и возвращает URL вида `/uploads/<file>`.
- Для проектов можно сразу подставить URL в `heroImage` или добавить в `gallery`.

Важно: в Vercel файловая система read-only для постоянной записи, поэтому загрузка в `public/uploads` там недоступна без внешнего storage (S3, Cloudinary и т.д.).
